...
//préparation des requêtes permettant d'afficher les menus déroulants
		$polluant = $connexion->prepare(
		'SELECT polluant FROM emissions group by polluant'
		);
...
?>	
	<h2>Consulter les données d'émissions atmosphériques</h2>
<p>Sélectionnez le ou les polluants</p>
<?php
// insertion du POST qui permet d'accéder à tableau_emi lorsqu'on clique sur le bouton valider
?>
<form method="POST" action="tableau_emi.php">
<select NAME=polluants[] size="3" multiple>
<?php
		while($ligne = $polluant->fetch()) {
			echo "<option value='".$ligne['polluant']."'>".$ligne['polluant']."</option>";
			}
?>
</select>
