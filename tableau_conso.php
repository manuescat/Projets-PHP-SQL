...
//TERRITOIRES	
	if(isset($_POST['territoires'])){
		...
		$join_geo = ' join territoire ter on (conso.id_commune=ter.id_commune)';
	}
//SECTEURS
	if(isset($_POST['secteurs'])){
		$column_sect = 'secteur';
		$select_sect = 'secteur';
		$join_sect = ' join secteur_activite sec on (conso.id_activite=sec.id_activite)';
...
//COMBUSTIBLES
	if(isset($_POST['combustibles'])){
		...
		$join_comb = ' join combustibles comb on (conso.combustible=comb.combustible)';
...
//REQUETE
		$select = "SELECT ".$select_geo.$coma_sect.$select_sect.$coma_comb.$select_comb.", round(sum(val_conso),2) as consommation_en_GWh";
		$from = " FROM ".$table;
		$where = " WHERE ".$where_geo.$and_sect.$where_sect.$and_comb.$where_comb;
		$groupby = " GROUP BY ".$groupby_geo.$coma_sect.$groupby_sect.$coma_comb.$groupby_comb." ASC";
